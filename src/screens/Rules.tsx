    ...
    <div className="mx-auto max-w-lg p-4">
      ...
      {query.trim().length >= 2 ? (
        <div className="grid gap-2">
          {hits.map((h, i) =>
            h.keywordId ? (
              <button key={i} type="button" onClick={() => openKeyword(h.keywordId!)} className="mf-torn-card p-3 text-left">
                <div className="font-bold">{h.title}</div>
                <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>{h.snippet}</div>
              </button>
            ) : (
              <Link key={i} href={h.href!} className="mf-torn-card p-3">
                <div className="font-bold">{h.title}</div>
                <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)' }}>{h.snippet}</div>
              </Link>
            ),
        ...
        <>
          <div className="grid gap-1.5">
            {ruleSections.map((r, i) => (
              <Link key={r.id} href={`/rules/${r.id}`} className="mf-torn-card flex items-center gap-3 px-3 py-2.5 font-semibold">
